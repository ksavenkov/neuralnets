...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  hid_output = logistic(model.input_to_hid * data.inputs);    %   <number of hidden units> by <number of data cases>
  class_input = model.hid_to_class * hid_output;     % <number of classes> by <number of data cases>
  class_normalizer = log_sum_exp_over_rows(class_input); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  ...
  ret.input_to_hid = (1/N) * ( (transpose(model.hid_to_class) * (class_output - data.targets)) .* hid_output .* (1 - hid_output) ) * transpose(data.inputs);
  ret.hid_to_class = (1/N) * (class_output - data.targets) * transpose(hid_output);

  % account for the weight-decay
